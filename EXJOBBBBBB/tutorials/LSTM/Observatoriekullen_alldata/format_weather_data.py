...
def format_lufttemp_8years():

    global lufttemp_8years

    lufttemp_full = pd.read_csv("lufttemp.csv",sep=';',parse_dates=['Representativt dygn'])
    ...
    for i,month_num in enumerate(month_list):
        ...
        if month_num != lastmonth:
            if (month_num == 5 or month_num == 11):
...
def format_neder_8years():

    global rain_8years

    neder_full = pd.read_csv("neder.csv",sep=';')
